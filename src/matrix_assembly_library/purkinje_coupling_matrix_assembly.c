...
//Added by Leto Riebel
ASSEMBLY_MATRIX(stem_cells_with_purkinje_coupling_assembly_matrix) {
    // [TISSUE]
    uint32_t num_active_cells = the_grid->num_active_cells;
    struct cell_node **ac = the_grid->active_cells;
    ...
    for(i = 0; i < num_active_cells; i++) {
        ...
        } else if((INFARCT_ZONE(ac[i]) >= 3) || (IS_PACI(ac[i]) >= 4) || (infarct_stage >= 4) || (LAYER(ac[i]) >= 4)) {
