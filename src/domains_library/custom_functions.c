...
//The 7th column in the mesh is coded as follows:
//1 = healthy endocardial, 2 = healthy midmyocardial (not used here), 3 = healthy epicardial
//4 = infarct endocardial, 5 = infarct midmyocardial (not used here), 6 = infarct epicardial
//7 = border zone endocardial, 8 = border zone midmyocardial (not used here), 9 = border zone epicardial
//10 = ventricular-like iPSC-CM, 11 = atrial-like iPSC-CM, 12 = nodal-like iPSC-CM
//The 8th column is the apicobasal gradient (IKs scaling factor)
//The 9th column the fast endocardial layer (assigned but not used here)
void set_model_with_stem_cells_custom_data(struct cell_node *cell, real_cpu *custom_data, uint8_t num_custom_data) {
    if(custom_data[0] > 9) {
        IS_PACI(cell) = custom_data[0] - 9;
    } else {
...
uint32_t set_model_with_stem_cells(struct grid *the_grid, const char *mesh_file, uint32_t num_volumes, double start_h, uint8_t num_custom_data,
    ...
    FOR_EACH_CELL(the_grid) {
        ...
        if(x > maxx || y > maxy || z > maxz || x < minx || y < miny || z < minz) {
            ...
            if(index != -1) {
                ...
                INITIALIZE_PACI_INFO(cell);
                if(load_custom_data) {
                    set_custom_data_for_mesh(cell, custom_data[cell->original_position_in_file], num_custom_data);
                }
...
SET_SPATIAL_DOMAIN(initialize_model_with_infarction) {
    ...
    uint32_t test_cube = 0;
    GET_PARAMETER_NUMERIC_VALUE_OR_USE_DEFAULT(uint32_t, test_cube, config, "test_cube");

    the_grid->start_discretization = SAME_POINT3D(start_h);
    the_grid->max_discretization = SAME_POINT3D(max_h);

    return set_model_with_stem_cells(the_grid, mesh_file, total_number_mesh_points, start_h, 3, &set_model_with_stem_cells_custom_data);
}
